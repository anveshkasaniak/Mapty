  ...
  async _newWorkout(e) {
    ...
    const getWeather = async function (lat, lng) {
      const response = await fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lng}&appid=7d58a3fa6d2aa94ae3b99928bf6c04c6`
      );
    ...
    const getLocation = async function (lat, lng) {
      const response = await fetch(
        `https://us1.locationiq.com/v1/reverse.php?key=pk.48d7339e11cc08a0d7516cfa0c4e38b1&lat=${lat}&lon=${lng}&normalizeaddress=1&format=json`
      );
      if (!response.ok) return;
      const data = await response.json();
